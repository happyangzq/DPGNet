...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        if mode == 'unlabeled':
            self.frame_num = config['frame_num']['unlabeled']
        else:
        ...
        if mode == 'train':
            ...
            if self.lmdb:
                if len(dataset_list) > 1:
                    if all(item in FFpp_pool for item in dataset_list):
                        ...
                        raise ValueError('Training with multiple datasets and lmdb is not implemented yet.')
                else:
            ...
            dataset_list = config['target_dataset']
            image_list = []
            label_list = []
            for one_data in dataset_list:
                tmp_image, tmp_label = self.collect_img_and_label_for_one_dataset(one_data)
            ...
            raise NotImplementedError('Only train, test, and unlabeled modes are supported.')
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        for label in dataset_info[dataset_name]:
            if self.mode == 'unlabeled':
                sub_dataset_info = dataset_info[dataset_name][label]['train']
            else:
    ...
    def load_rgb(self, file_path):
        ...
        if hasattr(self, 'env'):  # if use LMDB
            with self.env.begin() as txn:
                img_data = txn.get(file_path.encode('ascii'))
                if img_data is None:
                    raise ValueError(f"Image not found in LMDB: {file_path}")
                img = cv2.imdecode(np.frombuffer(img_data, np.uint8), cv2.IMREAD_COLOR)
        else:  
            full_path = os.path.join(self.config['rgb_dir'], file_path)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train' and self.config['use_data_augmentation']:
            ...
            xray_trans, patch_label_trans,clip_patch_label_trans = deepcopy(mask), deepcopy(mask), deepcopy(mask)
        else:
    ...
    def collate_fn(batch):
      ...
      #  labels = list(labels)  # Keep as list for unlabeled mode
        if isinstance(batch[0], dict) and 'label' not in batch[0]:
